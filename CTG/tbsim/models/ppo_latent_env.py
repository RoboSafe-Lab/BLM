...
class PPOEnv(gym.Env):
    def __init__(self,cfg, dataset, model,vae, ddim_steps):             
        super().__init__()
        ...
        self.T = int(cfg.future_num_frames /cfg.vae['vae_sample_stride'])
        self.vae_latent_dim = cfg.vae['vae_latent_dim']
        ...
        self.observation_space = spaces.Dict({
            "z_t":               spaces.Box(-np.inf, np.inf, shape=(self.T, self.vae_latent_dim), dtype=np.float32),
    ...
    def reset(self, seed=42, options=None):
        ...
        # —— 4) 初始化 DDIM 时间表 —— 
        self.timesteps, self.next_timesteps = self.model.make_ddim_timesteps()
        
        self.t_idx = 0
        # —— 5) 初始化噪声 x_T —— 

        self.z_t = torch.randn((1, self.T, self.vae_latent_dim), device=self.device)
    ...
    def step(self, action):
        ...
        if terminated:
            ...
            x_t = self.vae.vae_decoder(z_bld=self.z_t,
                                        cond_feat=None,
                                        grid_map_traj_T=None) 
            pred_pos = x_t[..., :2]  # [1,T,2]
