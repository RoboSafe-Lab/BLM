...
from tbsim.datasets.cld_datamodules import CLDDataModule
...
def datamodule_factory(cls_name: str, config):
    ...
    if cls_name.startswith("L5"):  # TODO: make this less hacky
        ...
        datamodule = eval(cls_name)(data_config=trajdata_config, train_config=config.train)
    elif cls_name.startswith("CLD"):
        cld_config = translate_cld_cfg(config)
        datamodule = eval(cls_name)(data_config=cld_config, train_config=config.train)
    else:
